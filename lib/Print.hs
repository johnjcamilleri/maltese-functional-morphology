...
prLMF l d =  unlines [
 ...
 pr (id,stem,para,cat,inhs,tbl,extr) =  ["<LexicalEntry>",
                                          " <Lemma>",
                                          "  <FormRepresentation>",
                                          "   <feat att=\"writtenForm\" val=\"" ++ amp stem ++ "\"/>",
                                          "   <feat att=\"lemgram\" val=\"" ++ id ++ "\"/>",
                                          "   <feat att=\"partOfSpeech\" val=\"" ++ cat ++ "\"/>",
                                          "   <feat att=\"paradigm\" val=\"" ++ para ++ "\"/>",
                                          unlines ["   <feat att=\"inherent\" val=\"" ++ i ++ "\"/>" | i <- inhs] ++ "  </FormRepresentation>",
                                          " </Lemma>"
                                        ]
                                      ++
                                      (concat 
                                     [[" <WordForm>",
                                       "  <feat att=\"writtenForm\" val=\"" ++ amp w ++ "\"/>",
                                       "  <feat att=\"msd\" val=\"" ++ t ++ "\"/>",
...
prRDF l d =  unlines [
  ...
  "  xmlns:lh=\"http://localhost/rdf-syntax#\">",
 ...
 pr (id,stem,para,cat,inhs,tbl,extr) =  
     concat [
         unlines 
         [
          "   <rdf:Description rdf:about=\"http://localhost/form/" ++ (urlEncode w) ++ "\">",
          "    <lh:id>" ++ id          ++ "</lh:id>",
          "    <lh:cf>"  ++ stem           ++ "</lh:cf>",
          "    <lh:p>" ++ para        ++ "</lh:p>",
          "    <lh:param>" ++ p              ++ "</lh:param>",
          "    <lh:pos>"   ++ cat            ++ "</lh:pos>",
          (concat ["    <lh:inhs>"  ++ i ++ "</lh:inhs>\n" | i <- inhs]) ++ "   </rdf:Description>"
...
prJSON = unlines . map snd . prJSONP
  
prJSONP :: Dictionary -> [(String,String)]
prJSONP = concat . map prOne . unDict
...
getSymbols :: Dictionary -> [String]
getSymbols = Set.toList . Set.unions . map collect . unDict
 where collect (id,stem,p,wc,inhs,tbl,_) = 
         Set.fromList (wc:p:concat [(inhs ++ words a) | (a,(_,s)) <- tbl, not (null (unStr s)), x <- unStr s])

-- | Print LexC source code
prLEXC :: Dictionary -> String
prLEXC d = 
  let symbols = getSymbols d in 
  unlines [
  "Multichar_Symbols " ++ (concat (intersperse " " (map ("+"++) symbols))),
  ("LEXICON Root\n" ++) $ (++ "END") $ unlines $ map (uncurry prLEXCRules) $ classifyDict d
  ]
  
prLEXCRules :: Ident -> [Entry] -> String
prLEXCRules cat entries = concat $
    ("\n! category " ++ cat ++ "\n") : (map prEntry entries)
  where
    prEntry e@(id,stem,p,cat,inhs,tbl,_) = 
      concat [prForm e (param, word) | (param,(attr,str)) <- tbl, word <- unStr str] 
    prForm (id,stem,p,cat,inhs,_,_)  (param,word) = 
      lexcstr (word ++ ":+" ++ cat ++ prTags [add_inhs (plus inhs) (unwords (plus (words param))), "+" ++ p,stem,id]) ++ " # ;\n"
    prTags ts = 
      concat ["@" ++ prFlat t | t <- ts]
    plus xs = map ("+"++) xs
    add_inhs [] param = lexcparam param
    add_inhs (x:xs) param = lexcparam $ param ++ "." ++ concat (intersperse "." (x:xs))
    lexcstr   = map (\c -> if c == ' ' then '-' else c)
    lexcparam = map (\c -> if c == ' ' then '.' else c)
...
-- | Print fst
prFST :: Dictionary -> String
prFST d = unlines [crossproduct x y | (x,y) <- prJSONP d]
  where 
